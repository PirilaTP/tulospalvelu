...
void __fastcall TFormCfg::FormCreate(TObject *Sender)
{
	...
	TulkitseTiedot(true);
	Memo1->Lines->Add(L"//CfgEditorAlku - Konfiguraatioeditori ei tulkitse tai muuta t�t� rivi� edelt�t�vi� rivej�");
	Memo1->Lines->Add(L"//CfgEditorLoppu - Konfiguraatioeditori ei tulkitse tai muuta t�t� rivi� seuraavia rivej�");
...
void TFormCfg::PaivitaIkkuna(void)
{
	...
	if (Loppu == 0) {
		Memo1->Lines->Insert(Alku, L"//CfgEditorLoppu - Konfiguraatioeditori ei tulkitse tai muuta t�t� rivi� seuraavia rivej�");
		}
	if (Alku == 0) {
		Memo1->Lines->Insert(0,L"//CfgEditorAlku - Konfiguraatioeditori ei tulkitse tai muuta t�t� rivi� edelt�t�vi� rivej�");
		Alku = 1;
		Loppu++;
		}
	r = Alku;
	...
	if (CBAani->ItemIndex != 1) {
		Memo1->Lines->Insert(r++, UnicodeString(L"��NI=")+UnicodeString(CBAani->ItemIndex));
		Loppu++;
		}
	...
	if (val != 1000) {
		Memo1->Lines->Insert(r++, L"LIS��="+UnicodeString(val));
		Loppu++;
		}
	...
	if (RGKunto->ItemIndex != 2) {
		Memo1->Lines->Insert(r++, RGKunto->ItemIndex == 0 ? UnicodeString(L"KUNTOMAALI") : UnicodeString(L"KUNTOL�HT�"));
		Loppu++;
		}
	if (CBHenkTietokanta->Checked) {
		US = EdHenkTietokanta->Text.Trim();
		if (US.Length() > 0 && wmemcmpU(L"henkilot.csv", US.c_str(), 12))
			Memo1->Lines->Insert(r++, UnicodeString(L"HENKIL�TIETOKANTA=")+US);
		else
			Memo1->Lines->Insert(r++, UnicodeString(L"HENKIL�TIETOKANTA"));
		Loppu++;
	...
	if (val != 0) {
		Memo1->Lines->Insert(r++, L"LIS�VAKANTTIALKU="+UnicodeString(val));
		Loppu++;
		}
	...
	if (CBValita->ItemIndex == 1) {
		Memo1->Lines->Insert(r++, UnicodeString(L"V�LIT�=EI"));
		Loppu++;
		}
	for (int y = 0; y < SG1->RowCount-1; y++) {
		...
		if (SG1->Cells[6][y+1].Pos(L"Aik") != 0) {
			Memo1->Lines->Insert(r++, L"L�HAIKA"+UnicodeString(ny));
			Loppu++;
			}
		if (SG1->Cells[6][y+1].Pos(L"AiV") != 0) {
			Memo1->Lines->Insert(r++, L"L�HAIKA"+UnicodeString(ny)+L"=V");
			Loppu++;
			}
		if (SG1->Cells[6][y+1].Pos(L"Emt") != 0) {
			Memo1->Lines->Insert(r++, L"L�HEMIT"+UnicodeString(ny));
			Loppu++;
			}
		if (SG1->Cells[6][y+1].Pos(L"EmI") != 0) {
			Memo1->Lines->Insert(r++, L"L�HEMIT"+UnicodeString(ny)+L"=I");
			Loppu++;
			}
		if (SG1->Cells[6][y+1].Pos(L"EmO") != 0) {
			Memo1->Lines->Insert(r++, L"L�HEMIT"+UnicodeString(ny)+L"=O");
			Loppu++;
			}
		if (SG1->Cells[6][y+1].Pos(L"EmV") != 0) {
			Memo1->Lines->Insert(r++, L"L�HEMIT"+UnicodeString(ny)+L"=V");
			Loppu++;
			}
		if (SG1->Cells[6][y+1].Pos(L"EVa") != 0) {
			Memo1->Lines->Insert(r++, L"L�HEMVA"+UnicodeString(ny));
			Loppu++;
			}
		if (SG1->Cells[6][y+1].Pos(L"ECd") != 0) {
			Memo1->Lines->Insert(r++, L"L�HECODE"+UnicodeString(ny));
			Loppu++;
			}
		if (SG1->Cells[6][y+1].Pos(L"Tie") != 0) {
			Memo1->Lines->Insert(r++, L"L�HTIEDOSTOT"+UnicodeString(ny)+
				(CBLahKaikkiPolku->Checked ? L"/K" : L""));
			Loppu++;
			}
		if (SG1->Cells[7][y+1].Pos(L"Ei") != 0) {
			Memo1->Lines->Insert(r++, L"V�LIT�"+UnicodeString(ny)+L"=Ei");
			Loppu++;
			}
		}
	...
	if (CBLahTiedostot->Checked) {
		Memo1->Lines->Insert(r++, UnicodeString(L"L�HTIEDOSTOT") + (CBLahKaikkiPolku->Checked ? L"/K" : L""));
		Loppu++;
		}
	...
	if (RGPiste->ItemIndex > 0) {
		...
		switch (RGPiste->ItemIndex) {
			...
			case 3:
				...
				break;
			case 4:
				Memo1->Lines->Insert(r++, UnicodeString(L"L�HDEPISTEET"));
				Loppu++;
				break;
			}
	...
	if (CBVainVA->Checked) {
		Memo1->Lines->Insert(r++, UnicodeString(L"VAINV�LIAJAT"));
		Loppu++;
		}
	...
	if (CBAikaLukija->ItemIndex > 0) {
		switch (CBAikaLukija->ItemIndex) {
			...
			case 4:
				if (EdAikaLukija->Text.Trim().Length() > 0) {
					Memo1->Lines->Insert(r++, UnicodeString(L"AIKALUKIJA=L�HDE")+EdAikaLukija->Text.SubString(1,1));
					Loppu++;
					}
				break;
			}
	...
	if (CBNollaKarki->Checked) {
		Memo1->Lines->Insert(r++, UnicodeString(L"NOLLAK�RKI"));
		Loppu++;
		}
	if (CBAutoEtuSuku->Checked == false) {
		Memo1->Lines->Insert(r++, UnicodeString(L"HTMLS�ILNIMI"));
		Loppu++;		
		}
	if (CB25rivi->Checked) {
	...
	if (EdVarit->Text.Trim().Length() > 3) {
		Memo1->Lines->Insert(r++, UnicodeString(L"V�RIT=") + EdVarit->Text.Trim());
		Loppu++;
		}
	...
	if (CBUudenTulos->Checked) {
		Memo1->Lines->Insert(r++, UnicodeString(L"SUORASY�TT�"));
		Loppu++;		
		}
	...
	if (EdNappain->Text.Trim().Length() > 2 && EdNappain->Text.Trim() != L"0,0") {
		Memo1->Lines->Insert(r++, UnicodeString(L"N�PP�IN=") + EdNappain->Text.Trim());
		Loppu++;
		}
...
//				upcasewstr(fldn);
		...
		else if (!wmemcmpU(fldn, L"P�IV�=", 6) || !wmemcmpU(fldn, L"VAIHE=", 6)) {
			Tulkitut[i] = 1;
		...
		else if(!wmemcmpU(fldn, L"V�RIT=", 6)) {
			Tulkitut[i] = 1;
			EdVarit->Text = fldn+6;
			continue;
			}
		...
		else if(!wmemcmpU(fldn, L"HENKIL�TIETOKANTA", 9)) {
			Tulkitut[i] = 1;
		...
		else if(!wmemcmpU(fldn, L"KUNTOL�HT�", 8)) {
			Tulkitut[i] = 1;
			RGKunto->ItemIndex = 1;
			continue;
			}
		...
		else if(!wmemcmpU(fldn, L"LIS�VAKANTTIALKU=", 7)) {
			Tulkitut[i] = 1;
			if ((p = wcsstr(fldn, L"=")) != NULL) {
		...
		else if (!wmemcmpU(fldn, L"V�LIT�",6) && (pos = 6) != 0 &&
		...
		else if (!wmemcmpU(fldn, L"L�HTIEDOSTOT",12)) {
			Tulkitut[i] = 1;
			pos = 12;
		...
		else if (!wmemcmpU(fldn, L"KUNTO",5) || !wmemcmpU(fldn, L"SUORASY�TT�",5)) {
			Tulkitut[i] = 1;
		...
		else if(!wmemcmpU(fldn, L"N�PP�IN=", 8)) {
			Tulkitut[i] = 1;
		...
		else if( !wmemcmpU(fldn, L"L�HAIKA",7) && (pos = 7) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
			Tulkitut[i] = 1;
			for (y = 1; y < SG1->RowCount; y++) {
				if (pos == 6 || (SG1->Cells[0][y].Length() > 0 && SG1->Cells[0][y].ToInt() == ny)) {
		...
		else if( !wmemcmpU(fldn, L"L�HECODE",8) && (pos = 8) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
			Tulkitut[i] = 1;
			for (y = 1; y < SG1->RowCount; y++) {
				if (pos == 6 || (SG1->Cells[0][y].Length() > 0 && SG1->Cells[0][y].ToInt() == ny)) {
		...
		else if( !wmemcmpU(fldn, L"L�HDEPISTEET",8)) {
			Tulkitut[i] = 1;
			RGPiste->ItemIndex = 4;
			continue;
			}
		...
		else if( !wmemcmpU(fldn, L"VAINV�LIAJAT",12)) {
			Tulkitut[i] = 1;
		...
		else if( !wmemcmpU(fldn, L"SJBOX",5) || !wmemcmpU(fldn, L"L�HDEPISTEET",8)) {
			Tulkitut[i] = 1;
			RGPiste->ItemIndex = 4;
			continue;
			}
		...
		if( !wmemcmpU(fldn, L"L�HEMIT",7) && (pos = 7) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
			int r = 0;
			Tulkitut[i] = 1;
			for (r = 1; r < SG1->RowCount-1; r++)
		...
		else if( !wmemcmpU(fldn, L"L�HEMVA",7) && (pos = 7) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
			int r = 0;
			Tulkitut[i] = 1;
			for (r = 1; r < SG1->RowCount-1; r++)
...
/*
		...
		else if (!wmemcmp(fldn, L"HTMLS�ILNIMI",9)) {
		...
		else if(!wmemcmp(fldn, L"L�HET�CR=EI", 10)) {
		 ...
		 if (!wmemcmp(fldn, L"SQLTALLENNUS", 7)) {
			if(!wmemcmp(fldn, L"LIS��EDELLE", 8)) {
		 ...
		 if(!wmemcmp(fldn, L"EST�MUUTOS", 8)) {
		 ...
		 if( !wmemcmp(fldn, L"YHTEISL�HT�",7)) {
		 if( !wmemcmp(fldn, L"ASAKKO=",7)) {
		 if( !wmemcmp(fldn, L"MMSAKKO=",8)) {
		 if( !wmemcmp(fldn, L"J�RJESTYS=KSA",13)) {
		 if( !wmemcmp(fldn, L"J�RJESTYS=KAS",13)) {
		 if( !wmemcmp(fldn, L"J�RJESTYS=AKS",13)) {
		 if( !wmemcmp(fldn, L"J�RJESTYS=ASK",13)) {
		 if( !wmemcmp(fldn, L"J�RJESTYS=SAK",13)) {
		 if( !wmemcmp(fldn, L"J�RJESTYS=SKA",13)) {
		 if( !wmemcmp(fldn, L"SOTILAS", 7)) {
			 if (!wmemcmp(fldn, L"J�RJESTYS=",10)) {
			if (!wmemcmp(fldn, L"MERI5MM",7)) {
		 if ((!wmemcmp(fldn, L"SIIMPORT", 8)) && (pos = 8) != 0 &&
		 if (!wmemcmp(fldn, L"HYVKAIKKI",9)) {
		 if (!wmemcmp(fldn, L"MONITORV�LI=",12)) {
		 ...
		 if( !wmemcmp(fldn, L"L�HT�RYHM�=",11)) {
		 if (!wmemcmp(fldn, L"TAUSTA-AIKA", 8)) {
		 if( !wmemcmp(fldn, L"AIKAKORJAUS",11)) {
		 if( !wmemcmp(fldn, L"L�HKELLO",8) && (pos = 8) != 0 &&
		 if( !wmemcmp(fldn, L"SY�TT�", 6)) {
		 if( !wmemcmp(fldn, L"AIKA_ESTO=",10)) {
		 if (!wmemcmp(fldn, L"KELLO_ESTO=",11)) {
		if (!wmemcmp(fldn, L"EST�NEG",7)) {
		if (!wmemcmp(fldn, L"EST�HAAMUT",7)) {
		 if (!wmemcmp(fldn, L"EST�YLIM",8)) {
		 ...
		 if (!wmemcmp(fldn, L"L�HT�IKKUNA=", 12)) {
		 ...
		 if( !wmemcmp(fldn, L"EIL�HT=AVOIN", 8)) {
		 if( !wmemcmp(fldn, L"EMLOKI",5)) {
		 if( !wmemcmp(fldn, L"ESITARK",5)) {
		 if( !wmemcmp(fldn, L"L�HT�VERT=",10)) {
			if (!wmemcmp(fldn, L"EMITAGLIS�=0", 12)) {
			if (!wmemcmp(fldn, L"EMITAGLIS�", 9)) {
			if (!wmemcmp(fldn, L"ECAIKA",4)) {
			if (!wmemcmp(fldn, L"ETSQL", 5)) {
			if (!wmemcmp(fldn, L"ETGPRS", 6)) {
			if (!wmemcmp(fldn, L"ETHAKUV�LI=",11)) {
...
void __fastcall TFormCfg::LaiteAjatClick(TObject *Sender)
{
	if (Sender == Eiaikoja1)
		...
		SGLeimat->Cells[SGLeimat->Col][SGLeimat->Row] = L"L�HDExx";
}
//---------------------------------------------------------------------------
