...
// Konsolipohjainen viestikilpailun tulosseuranta, EMIT-käsittely ja yhteyksien hallinta.
...
void outkilp(kilptietue *kilp, int osuus)
   ...
   viwrrect(YAY,YAX-19,YAY,YAX-2,"Sarjan l�ht�aika :",7,0,0);
...
//               vay = osr+1+(va > 4);
	  ...
	  if(kilp1.ostiet[os].ylahto != TMAALI0) {
		 ...
		 if (os == osuus) {
			viwrrect(YAY,YAX-20,YAY,YAX-2,"Yhteisl�hd�n aika :",7,0,0);
			viwrrect(YAY,YAX,YAY,YAX+7,
						   ...
						   if (bdg != kilp->ostiet[*osuus].badge[0]) {
							  if (!emitfl)
								 writeerror("Emit-toimintoa ei k�ynnistetty - "
									"haitallisia sy�tt�virheit� ei estet�", 0);
							  ...
							  if (ch != ESC && o2 > 0 &&
								 o2 <= Sarjat[kilp->sarja].osuusluku) {
								  if (kilp->ostiet[o1 - 1].lisno != kilp->ostiet[o2 - 1].lisno) {
									  ch = ' ';
									  selectopt("Vaihdetaanko my�s Lisenssinumerot (K/E)",
										  "KE", &ch);
									  if (ch == 'K') {
								  ...
								  if (kilp->ostiet[o1 - 1].badge[0] != kilp->ostiet[o2 - 1].badge[0] || 
									...
									ch = ' ';
									selectopt("Vaihdetaanko my�s Emit-koodit (K/E)",
									   "KE", &ch);
									if (ch == 'K') {
...
/*
   ...
   if (!tarfl) {
      writeerror("Tarratulostus ei k�yt�ss� t�ll� koneella", 2000);
      return;
      }
...
/*
      if (MAY == maikay &&
         ...
         erbeep();
         ch = ' ';
         selectopt("Maaliaika muuttunut yli 15 min - Hyv�ksyt��nk� (K/E)?",
            "KE", &ch);
         if (ch == 'E') goto lue;
         }
*/
...
//           else writeerror("Yhteisl�ht�automatiikka k�yt�ss�", 0);
            kh = 0;
            }
#if !defined(MAALI) && VAIKALUKU > 0
...
static void luejuoksujarj(void)
   ...
   if (tiedots == 1) {
      do {
         if (!fgets(line, 198, efile)) {
            writeerror("Otsikkorivi� ==ILMOITTAUTUMISET ei l�ytynyt",0);
            fclose(efile);
            efile = NULL;
            return;
            }
   ...
   if (lueuusirivi(MAXOSUUSLUKU+1)) {
      writeerror("Tiedostosta ei l�ydy ilmoittautumisia",0);
      fclose(efile);
      efile = NULL;
      return;
      }
      ...
      if ((d = getpos(kno)) > 0) {
         ...
         switch (ptime.kno) {
            case 10000: wmemcpy(line+28,L"TAHDISTUSAIKA", 13);
                        break;
            case 19998: wmemcpy(line+28,L"NOLLAUS - YLIM��R�INEN AIKA", 27);
                        break;
			...
			default:
				if (ptime.kno > 30000 && ptime.kno <= 30000 + sarjaluku) {
					...
					wmemcpy(line + 23, L"1 L  Sarjal�ht�: ", 17);
				...
				if (ptime.kno > 31000 && ptime.kno < 32000) {
					...
					wmemcpy(line + 23, L"1 L  Er�l�ht�: ", 15);
            ...
            case 1 :
               ...
               do {
				  ...
				  if (tc == L'S') {
					  ...
					  if (ch != ESC && srj1 >= 0) {
						  ch = ' ';
						  selectopt("Sarjan kaikkien joukkueiden l�ht�aika vaihdetaan. Vahvista (K/E)", "KE", &ch);
						  if (ch == 'K') {
							  utime = vert_time;
				  ...
				  if (tc == L'R') {
					  ...
					  if (ch != ESC && ryh > 0) {
						  ch = ' ';
						  selectopt("Er�n kaikkien joukkueiden l�ht�aika vaihdetaan. Vahvista (K/E)", "KE", &ch);
						  if (ch == 'K') {
							  utime = vert_time;
...
void lahestyjat(void)
{
	...
	vidspmsg(ySize-3, 0, 7, 0, "  Esc: palaa ajanotton�yt�lle                               ");
