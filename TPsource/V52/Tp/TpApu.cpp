...
void kirj_err_file(wchar_t *msg, int nayta)
	...
	if (nayta && !lopilm) {
		...
		wcscpy(msg2, L"J)atka, L)opeta n�m� virheilmoitukset");
		...
		swprintf(msg2, L"%s \nJatketaanko n�iden virheilmoitusten n�ytt�mist�", msg);
		vast = select3(2, msg2, L"Toistuva virheilmoitus", L"N�yt� edelleen", L"Ohita ilmoitukset jatkossa", L"", 0);
		if (vast == 2)
			lopilm = 1;
#endif
		}
...
void close_err_file(void)
	...
	if (err_file) {
		...
		writeerror_w(msg, 0);
#else
		if (select3(2, msg, L"Virheit� tiedoissa", L"N�yt� tiedot", L"Ohita", L"", 0) == 1)
			nayta_virhesanomat();
#endif
		}
	}
...
INT luesarja(wchar_t *snimi, wchar_t *tc, bool salliyhd)
	...
	do {
		vidspwmsg(ySize-1,0,7,0, L"Sarjoja voi selata n�pp�imill� \x19 \x18 PgDn PgUp");
		do {
			if (srj >= 0 && srj < nn)
...
INT luesarja(char *snimi, char *tc)
	...
	do {
		vidspmsg(ySize-1,0,7,0, "Sarjoja voi selata n�pp�imill� \x19 \x18 PgDn PgUp");
		do {
