...
// Aikajonojen hallinta: aikojen tallennus, poisto ja tiedostoon kirjoitus henkilökilpailussa.
...
//
//    ino      ajanottojonon numero (1 : l�ht�ajat, 0: muu)
...
//             tietoja sy�tet��n ko. tietokoneelta
...
//		 if (NORMTULOS_A(tm.t - vtm->t) == 0 && tm.kno == vtm->kno &&
		 if (it2 == it1 && tm.kno == vtm->kno &&
		  ...
		  // T�ss� on l�ydetty tietue, jossa aika, kilpailija ja piste oikeat
		  // Jos L'kysy' voimassa ja rivi vaihtuu kysyt��n vahvistusta
...
//						NORMTULOS_A(utm->t - gettm(rtm-1, 0, ino)->t) >= 12L*TUNTI*AIKAJAK
				...
				ch = L' ';
				wselectopt(L"Aika ei vastaa rivi� - vahvista tallennus (K/E)",
					L"KE", &ch);
				if (ch == L'E') {
					if (loki)
		...
		// T�h�n tullaan, jos looppi on k�yty loppuun l�yt�m�tt� oikeaa rivi�
   ...
   // Seuraava lohko k�sitell��n, jos kirjataan uusia tietoja
	...
	if (utm && utm->t != TMAALI0*AIKAJAK) {
	  ...
	  if((aikajono[ino]->rwtime - aikajono[ino]->mxtime) >= 0) {
		 ...
		 if (loki) wkirjloki(L"Aikataulukko t�ysi, tallennus peruutettu");
		 errret = 1;
		 goto poistu;
			}
	  ...
	  for (rtm = aikajono[ino]->rwtime; rtm > 0 &&
		...
		//  Tallenna uusi tietue, ellei sit� tulkita aiemman tiedon toistoksi

		if (it2 != it1 || tm.kno != utm->kno || tm.badge != utm->badge ||
			(tm.kno == 0 && tm.badge == 0 && memcmp(&tm, utm, sizeof(aikatp)))) {

			// varmista, ett� aika poikkeaa aiemmista ajoista kasvattamalla aikaa, jos se on sama kuin aiemmin tallennettu
			while (it1 == it2) {
				utm->t++;
				it1 = datetime64(utm);
