...
// EMIT/Sirit-sirukorttilukijoiden ja rastitunnusten leimausten käsittely viestikilpailussa.
...
//   viwrrect(0,60,16,60,"�����������������",7,0,0);
//   viwrrect(16,61,16,65,"�����",7,0,0);
   for (r = 0; r < 16; r++) {
      memset(st, ' ', 19);
      fore = (r == 8) ? 0 : 7;
      if (seurkrivi + r >= 8) {
...
void lueemitkoodit(void)
   ...
   if ((emitkdfile = fopen(fname, "r")) == NULL) {
      writeerror("Tiedostoa EMITKDI.LST ei l�ydy", 0);
      return;
      }
   ch = ' ';
   selectopt("Merkit��nk� luetut koodit lainakorteiksi (K/E)", "KE", &ch);
   merklaina = ch == 'K';
   for (;;) {
...
//				p = initfont_emva;
		...
		if (partiorap == 2) {
			...
			if (tl > VRK)
				tl = NORMKELLO(tl);
			if (tulprm->language == 0)
				swprintf(line, L"L�ht�aika          %8.8s", aikatowstr_ls(as, tl, t0));
			else
...
void vaihdakilp(INT ekno, INT eos, INT vainos)
   ...
   if (ch == 'O') {
      ...
      os--;
      if ((du = getpos(ekno)) <= 0) {
         writeerror("Kilpailijaa ei l�ydy", 0);
         return;
         }
      GETREC(&kilp, du);
   ...
   for (;;) {
      ...
      do {
         ...
         for (;;) {
            if (!kbready(&ch, &key)) {
               ...
               // jos selaus ei ole k�yt�ss� ja emithead muuttuu, haetaan
               // viimeinen tietue
      ...
      if (emithead2 && (ch == 'S' || ch == 'D' || ch == 'V' || ch == 'L' || 
		 ...
		 if (kilpno && ((trk && trk != kilp.Tark(os1, 0)) ||
            ...
            cs = ' ';
            selectopt("T)allenna n�yt�n tiedot, O)hita muutokset, P)alaa tietoihin",
               "TOP", &cs);
            switch (cs) {
               case 'T' :
                  emit_tallenna(&kilp, os1, trk, selitys, 0);
      ...
      do {
         ...
         if ((d = getpos(kilpno)) < 1) {
			vidspmsg(3,0,7,0, "Kilpailijaa ei l�ydy");
			ch = 0;
            }
         else {
            GETREC(&kilp, d);
...
void emit_rap(tulostusparamtp *tulprm, INT kilpno, INT os)
   ...
   if (kokosivu) {
		if (tulprm->language > 0)
			...
			swprintf(line, L"L�ht�aika          %8.8s", AIKATOWSTRS(as, tl, t0));
		putfld(tulprm,line, 4*tulprm->tulmuot.numlev, wcslen(line), 0, 0);
		endline(tulprm,0);
		}
   tls = 0;
...
void tark_rap(tulostusparamtp *tulprm, INT kilpno, INT os)
   ...
   switch (ch) {
      ...
      case L'P' :
         if (!esitark || esitark == 2) {
            wselectopt(L"N)�yt�lle, P)aperille", L"NP", &cr);
