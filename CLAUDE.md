...
- **Webadmin is single-day (vaihe=1) for now.** Its ALKUT handshake always announces stage 1, and the C++ server rejects (`tark_alku` in `Hk/HkCom32.cpp`) a peer whose `vaihe != k_pv + 1`. So a webadmin connection only succeeds against a server currently on the **first** stage. `TulospalveluService.onPeerHandshake` parses the peer's ALKUT (`TulospalveluProtocol.parseAlkutData`) and records a `connectionWarning` (shown as a notification by `ConnectionStatusIndicator`) when the peer reports a different day or KILP.DAT record count. Multi-day support is a planned follow-up.
