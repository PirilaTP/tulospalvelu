...
// Viestikilpailun vakiomäärittelyt, kääntöparametrit ja tietotyypit (MAXOSUUSLUKU, MAXJONO, jne.).
...
//   M��ritysten HALIKKO jne. merkitys:
//
...
//             3. osuus l�htee 2. osuuden valmistuttua
//             Useita sarjoja
//             Oletusosuuden valinta
...
typedef struct {
	...
	int pyor[5];             // 0: ei, 1: tall, 2: jarj, 3: tuloste, 4: v�liaika
	bool sailhyl;
	int lnimi;
	int lseura;
	int lsakmax;
...
public:
	...
	int  piste;       // 0: maali, v�liaika
	int  enssija;      //
	int  viimsija;     //
	bool hksijafl;     //
	int yhteislahdot;     // yhteisl�ht��n osallistuneet, 2: kaikista sarjoista
	...
	hajtp hajrata;
	int  sj_raja;           // piiritulosten k�rki
	bool piilotatulos;      // ?
	bool piilotasijat;      // ?
	bool lahtoluettelo;     // ?
	bool lahtoajat;        // partion ym. l�ht�ajat
...
typedef struct {                      /* Tiedonsiirtopaketti */
		...
		union {
			struct {
				...
				UINT8		vaihe;			// kilpailun vaihe, viestiss� 0
		   ...
		   struct {
			  ...
			  UINT32		 muutos;	  // flagit, jotka kertovat sanoman sis�ll�n
