...
void TFormCfg::PaivitaIkkuna(void)
{
	...
	if (CBSailHyl->Checked) {
		Memo1->Lines->Insert(r++, UnicodeString(L"S�ILHYL"));
		Loppu++;
		}
	...
	if (CBNaytaHaj->Checked) {
		Memo1->Lines->Insert(r++, UnicodeString(L"N�YT�HAJONTA=EI"));
		Loppu++;
		}
	...
	for (int y = 0; y < SG1->RowCount-1; y++) {
		...
		if (SG1->Cells[6][y+1].Pos(L"AiP") != 0) {
			Memo1->Lines->Insert(r++, L"L�HAIKA"+UnicodeString(ny)+L"=P");
			Loppu++;
			}
		...
		if (SG1->Cells[7][y+1].Trim().Length() > 0) {
			if (SG1->Cells[7][y+1].Pos(L"Ei") != 0)
				...
				Memo1->Lines->Insert(r++, L"V�LIT�"+UnicodeString(ny)+L"="+SG1->Cells[7][y+1].Trim());
			Loppu++;
			}
		}
	...
	if (RGEmittoiminto->ItemIndex > 1) {
		Memo1->Lines->Insert(r++, UnicodeString(L"SIS��NLUENTA")+
			(RGEmittoiminto->ItemIndex == 3 ? L"=MY�S" : L""));
		Loppu++;
		}
	...
	if (CBJalkiSyotto->Checked) {
		Memo1->Lines->Insert(r++, UnicodeString(L"J�LKISY�TT�"));
		Loppu++;
		}
...
//				upcasewstr(fldn);
		...
		else if(!wmemcmpU(fldn, L"N�YT�HAJONTA=EI", 14)) {
			Tulkitut[i] = 1;
			CBVuorokaudet->Checked = true;
			continue;
			}
		...
		else if(!wmemcmpU(fldn, L"S�ILHYL", 8)) {
			Tulkitut[i] = 1;
...
*/
		...
		else if( !wmemcmpU(fldn, L"SIS��NLUENTA",8)) {
			Tulkitut[i] = 1;
		...
		else if( !wmemcmpU(fldn, L"J�LKISY�TT�",7)) {
			Tulkitut[i] = 1;
