...
// Tiedonsiirtofunktiot viestikilpailulle: RS-232, UDP ja TCP-viestintä.
   ...
   for (cn = 0; cn < maxyhteys; cn++) {
	  ...
	  if (!levypusk) {
		 ...
		 while (ch != ESC && v == cjens[cn]) {
			clrln(ySize-3);
#ifndef LANGSV_K
			vidspmsg(ySize-3,0,7,0,"Odottaa tiedonsiirron etenemist�.  'Esc' "
			   "lopettaa yhteisl�ht�aikojen siirron");
#else // LANGSV_K
			vidspmsg(ySize-3,0,7,0,"Odottaa tiedonsiirron etenemist�.  'Esc' "
			   "lopettaa yhteisl�ht�aikojen siirron");
#endif // LANGSV_K
...
// luokka & 4,    l�het��n osuustietoja, lis�koodia kerrottuna 16:lla
// luokka == 8,   l�het��n muut osuustiedot
//                paitsi, jos maalil�hetysrajoitus
//
...
static void tark_aika(int cn)
   ...
   if (!com_ajat[cn]) {
      writeerror("Vastaanotettu MAALI-ohjelman ajanottotieto."
		 " LIS�� PARAMETRI L�HAIKA", 0);
	  return;
      }

   // Jos ajanottoa ei ole viel� k�ynnistetty, sijoitetaan saapuva tieto
   // puskuriin initbuf odottamaan my�hemp�� k�sittely� ja poistutaan
   if (!timerfl) {
...
//			   (char *)inbuf + inbens*combufsize, 9+2*sizeof(aikatp));
   ...
   // Jos t�h�n tullaan, on ajanotto k�ynnistetty ja uusinta sanomaa ei ole
   // pantu puskuriin initbuf

   do {
      // ensin katsotaan onko puskurissa tietoja, jotka pit�� k�sitell� ensin
      // jos on, k�yd��n t�t� looppia l�pi, kunnes aiemmat on k�sitelty ja
      // sitten k�sitell��n se sanoma, jonka johdosta t�h�n aliohjelmaan tultiin
...
//		ibuf = (combufrec *)((char *)inbuf + inbens*combufsize);
	  ...
	  //  Viestiohjelmassa tallennetaan uusi osuusaika my�s kilpailija-
	  //  tietoihin t�m�n sanoman perusteella.
...
bool tark_alku(const combufrec * const cbuf, int cn)
{
	...
	if (cbuf->d.alku.vaihe != 0) {
		if (var[cn] == 0) {
			swprintf(ln, L"Henkil�kohtainsen kilpailun ohjelma pyyt�� yhteytt�", cn+1);
			writeerror_w(ln, 0);
			}
		var[cn]++;
		return(true);
...
//				   ibuf = (combufrec *)((char *)inbuf + inb*combufsize);
			   ...
			   switch (ibuf->pkgclass) {
...
#else // LANGSV_K
					 writeerror("Programmet MAALI har s�nt tidtagningsdata."
				  ...
				  case FILESEND:
					 ...
					 break;
                  default :
                     writeerror("Tuntematon sanomalaji, ohjelmat eiv�t sovi yhteen", 1000);
                     lisa = 0;
                     break;
				  }
               EnterCriticalSection(&inb_CriticalSection);
...
void uusintaTCP(int cn)
   ...
   if (cs != 'S') {
	  selectopt("T)ulokset, V)�liajat, M)olemmat, H)yl+kesk+avoimet, K)aikki", "TVMHK", &cp);
...
void emitva_uusinta(int cn, int tietue)
	...
	if (tietue < 0 && cn >= 0) {
		while (1) {
			ch = ' ';
			selectopt("L�het� K)aikki tai valitse S)arja, O)suus, M)olemmat, Esc: lopeta", "KSOM\x1b", &ch);
			if (ch == ESC)
				break;
			if (ch == 'S' || ch == 'M') {
				if (srj >= 0)
