...
void kirj_err_file(char *msg, int nayta)
   ...
   if (nayta && !lopilm) {
      ...
      ch = 'J';
#ifdef _CONSOLE
      sprintf(msg2, "J)atka, L)opeta n�m� virheilmoitukset");
#else	  
	  sprintf(msg2, "%s\nJ)atka, L)opeta n�m� virheilmoitukset", msg);
#endif	  
...
static void luekorkeat(void)
   ...
   do {
      ...
      viwrrect(16,0,16,72,"Anna levennetyn tekstin k�ynnistysmerkkijono "
...
static void nimio(void)
{
   ...
   viwrrect(2,19,2,60,"VIESTISUUNNISTUKSEN TULOSPALVELUOHJELMISTO",
	  7,0,0);
   viwrrect(4,34,4,45,"Pekka Piril�",7,0,0);
#ifndef LUENTA
   ...
   if (*nf == 0) {
		...
		if (!cfgfile->IsOpen()) {
			...
			if (kierros > 0 && cfgparam) {
				swprintf(line, L"Konfiguraatiotiedostoa %s ei l�ydy",
					cfgflnm);
				writeerror_w(line ,0);
				}
			return(1);
...
INT yhteys_no(wchar_t *st, INT *pos)
   ...
   if (no > MAX_LAHPORTTI) {
		writeerror_w(L"Yhteyden numero liian suuri t�lle ohjelmaversiolle", 0);
		no = 0;
		}
   return(no);
   }
...
//		 oemtoansi(fldn, 0);
		...
		if(!wmemcmp(fldn, L"KUULUTTAJA", 7)) {
		 ...
		 if (!wmemcmp(fldn, L"SAKOT",5)) {
			writeerror_w(L"Parametri SAKOT poistettu k�yt�st�. Katso ohjeita", 0, true);
			continue;
			}
		 if (!wmemcmp(fldn, L"AMPUSAKOT",9)) {
			writeerror_w(L"Parametri AMPUSAKOT poistettu k�yt�st�. Katso ohjeita", 0, true);
			continue;
			}
		 if(!wmemcmp(fldn, L"HIIHTO", 6)) {
			writeerror_w(L"Parametri HIIHTO poistettu k�yt�st�. Katso ohjeita", 0, true);
			continue;
			}
		 ...
		 if(!wmemcmp(fldn, L"SIS��NLUENTA", 9)) {
			sisaanluenta = 2;
		 ...
		 if(!wmemcmp(fldn, L"PARTIO", 6)) {
			 writeerror_w(L"Parametri PARTIO poistettu k�yt�st�. Katso ohjeita", 0, true);
			continue;
			}
		 if(!wmemcmp(fldn, L"ROGAI", 5)) {
			 writeerror_w(L"Parametri ROGAINING poistettu k�yt�st�. Katso ohjeita", 0, true);
            continue;
            }
...
*/
		 ...
		 if(!wmemcmp(fldn, L"EST�MUUTOS", 8)) {          //  Est�� tallennuksen 
			esta_tall = 1;
			continue;
			}
		 ...
		 if( !wmemcmp(fldn, L"RATAVAL=", 8)) {          // vaikuttaa hajontatiedon tulostuksen tilank�ytt��n
...
//            wcscat(autokomento, L" > NUL 2>> cmderrors.txt");
		 ...
		 if( !wmemcmp(fldn, L"TCPHAJONTA",6)) {   // M��r�� l�hetet��nk� hajonta TCP-sanomissa
			lahhajonta = 1;
			}
#endif
         ...
         if( !wmemcmp(fldn, L"PUSKUROI",8)) {   // immed: tiedostoonkirjoitus vied��n heti levylle (ei Textfl)
			immed = 0;
			if (fldn[8] == L'=')
               kirjheti = (UINT32) _wtoi(fldn+9);
            continue;
			}
		 if( !wmemcmp(fldn, L"S�ILHYL", 7)) {
         ...
         if( !wmemcmp(fldn, L"V�LIAJAT",8)) {   // Pyyt�� v�liaokojen k�sittely�. Yleens� tarpeeton
			if (!valiajat)
			   valiajat = -1;
         ...
         if (!wmemcmp(fldn, L"V�LIT�",6) && (pos = 6) != 0 &&
            (ny = yhteys_no(fldn, &pos)) != 0) {
			y = 0;
...
//            if (fldn[7] && (fldn[8] == L'0' || fldn[8] == L'E')) levypusk = 0;
         ...
         if( !wmemcmp(fldn, L"��NI=",5)) {
            aani = fldn[5] - L'0'; 
            continue;
            }
		 if( !wmemcmp(fldn, L"N�YT�HAJONTA=EI", 14)) {     // Vaikuttaa ViestiWinin osuustietoikkunaan
				naytahajonta = 0;
			continue;
            }
#ifdef DBGFILE
...
#endif
         if( !wmemcmp(fldn, L"L�HAIKA",7) && (pos = 7) != 0 &&
            (ny = yhteys_no(fldn, &pos)) != 0) {
            ajanottofl = 1;
            com_ajat[ny-1] = 1;
			if (fldn[pos+1] == L'V')
		 ...
		 if( !wmemcmp(fldn, L"L�HECODE",8) && (pos = 8) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
			lahec[ny-1] = 1;
			if (fldn[pos+1] == L'V')
				lahec[ny-1] = 2;
		 ...
		 if( !wmemcmp(fldn, L"L�HTIEDOSTOT",8)) {
			y = -1;
         ...
         if( !wmemcmp(fldn, L"MUUTOSKYSELY=", 13)) {       // MUUTOSKYSELY=E v�hent�� vahvistusten kysymist� ajanotossa
		 ...
		 if( !wmemcmp(fldn, L"VAINV�LIAJAT",12)) {
			vainvaliajat = 1;
			continue;
			}
		 ...
		 if (!wmemcmp(fldn, L"VUOROMAALI", 8)) {        // Antaa ajan maaliajaksi, kun v�liajat ovat t�ynn�, vaikka ei muuten antaisi
		 ...
		 if (!wmemcmp(fldn, L"VAROITUSKYNNYS=", 15)) {   // Kertoo, milloin varoitetaan poikkeavan hyv�st� osuusajasta
		...
		if( !wmemcmp(fldn, L"LAINAT",5)) {
         ...
         if( !wmemcmp(fldn, L"L�HEMIT",7) && (pos = 7) != 0 &&
            (ny = yhteys_no(fldn, &pos)) != 0) {
			emitfl = TRUE;
            com_emit[ny-1] = 1;
            if (fldn[pos] == L'=') {
         ...
         if( !wmemcmp(fldn, L"L�HEMVA",7) && (pos = 7) != 0 &&
            (ny = yhteys_no(fldn, &pos)) != 0) {
			emitfl = TRUE;
            com_emitva[ny-1] = 1;
			emitvafl = true;
         ...
         if( !wmemcmp(fldn, L"L�HDEPISTEET",8)) {
			lahdepistehaku = 1;
			continue;
			}
         ...
         if( !wmemcmp(fldn, L"EMITTO",6)) {          // varoittaa saman Emitkoodin k�yt�st� kahdella osuudella
		 ...
		 swprintf(msg, L"Tuntematon parametri %.17s %s %s", fldn,
			(fld == argv) ? L"komentorivill�" : L"tiedostossa ",
...
/*
	  ...
	  viwrrect(ySize-1,5,ySize-1,54,
		 "NUL : Ei tarroja,  L�HET� : Toisella tietokoneella",7,0,0);
   ...
   else if ( !memcmp(tarfname, "L�HET�", 4) ) {
	  tarfl = FALSE;
	  lahtarfl = TRUE;
...
*/
   ...
   if (ch) {
		selectopt("Merkist�: I)BM, A)scii-7, R)oman-8, P)ostscrpt"
			", W)indows,L)evylt�", "ARLIPWG",&merkit);
		if (merkit == 'W') {
			ch2 = ' ';
			selectopt("K�ytet��nk� Windowsin kirjoitinajureita (K/E)", "KE", &ch2);
			if (ch2 == 'K')
				merkit = 'G';
			}
		}
   clrln(ySize-3);
   switch (merkit) {
      case 'A' : viwrrect(*y,63,*y,78,"Merkist� Ascii-7",7,0,0);
                 break;
      case 'R' : viwrrect(*y,63,*y,78,"Merkist� Roman-8",7,0,0);
				 break;
      case 'P' : viwrrect(*y,63,*y,79,"Merkist� Postscr.",7,0,0);
                 break;
	  case 'W' : viwrrect(*y,63,*y,78,"Merkist� Windows",7,0,0);
                 break;
	  case 'L' : viwrrect(*y,63,*y,78,"Merkist� levylt�",7,0,0);
				 break;
      case 'G' : viwrrect(*y,63,*y,78,"Windowskirjoitin",7,0,0);
                 break;
	  default  : viwrrect(*y,63,*y,79,"Merkist� IBM (PC)",7,0,0);
      }
   ++*y;
   ++*y;
   if (ch) {
	  ...
	  selectopt("��nimerkki: E)i k�yt�ss�,  K)�yt�ss�" ,"EK", &ch2);
      switch (ch2) {
         case 'E' : aani = 0; break;
		 case 'K' : aani = 1; break;
         }
      }
   viwrrect(*y,5,*y,16,"��nimerkki :",7,0,0);
   switch (aani) {
	  case 0  : viwrrect(*y,18,*y,28,"Ei k�yt�ss�", 7, 0, 0); break;
      default : viwrrect(*y,18,*y,25,"K�yt�ss�", 7, 0, 0); break;
      }
   ++*y;
   strcpy(msg, "K�yt�ss� yhteydet:");
   for (int nc = 0; nc < MAX_LAHPORTTI; nc++) {
      if (ch && nc < 8) {
         clrln(ySize-3);
         viwrrect(ySize-3,5,ySize-3,46,
            "Anna portin   numero :    (0 : ei k�ytet�)",7,0,0);
	 ...
	 if( portparam[nc] != 0) {
        ...
        if (!vastcom[nc])
           vidspmsg(*y,53,7,0,"Vain l�hetys               ");
#if MAX_LAHPORTTI > 22
        if (strlen(msg) < 73)
#endif
   ...
   if (!bootfl) {
	  ...
	  vidspmsg(ySize-3,5,7,0,
		 "<Return>: hyv�ksy valinnat,  K)orjaa valintoja,  <Esc>: keskeyt�");
      do {
...
//			autoalku(autostr);
...
#ifndef _CONSOLE
		swprintf(msg,L"Tiedostoa %s ole. Luodaanko uusi tyhj� tiedosto ilmoittautumistilassa?",
			ansitowcs(st, dataf2name, 98));
   ...
   if (ToimintaTila < 2) {
	...
	if (emitfl) {
		...
		// Jos merkinn�iss� ei kirjattuja eroja, p�ivit� tiedot, joita ei viel� kirjattu
		if (rno == NREGNLY) {
			if (kaikki_ajat[0] >= 0)
				ka = kaikki_ajat[0];
			else
	   ...
	   for (i=0; i < sarjaluku; i++) {
		  for (y = 0; y < Sarjat[i].ntosuus; y++) {
			  if (Sarjat[i].ylkynnys[y] != TMAALI0) {
				  writeerror("VAROITUS: Ei tuloksia, mutta yhteisl�ht�automatiikka k�yt�ss�", 0);
				  i = sarjaluku;
				  break;
				  }
			  }
