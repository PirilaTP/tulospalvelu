...
// Viestikilpailun tulosten XML-kirjoitus sisältäen IOF 2.0 ja oman XML-muodon.
...
int lueVainRadatXml(wchar_t *filename, IOFCourseData *CseData)
{
	...
	if (depth) {
		er = 1;
		swprintf(msg, L"XML-tiedoston %s lukeminen p��ttyi rivill� %d tasolla %d, node %d", filename, rv, depth, inode);
		}
	if (er) {
		if (!msg[0]) {
...
#ifdef _DEBUG
	swprintf(msg, L"XML-ratatiedostosta luettu %d rivi�, 10000*%d nodea", rv, jt);
	writeerror_w(msg, 0);
#endif
