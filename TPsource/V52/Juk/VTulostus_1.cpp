...
// Viestikilpailun tulostuksen lisätoiminnot: pistesarakkeet, sarjan otsikot ja tulosrivien muotoilu.
...
/*
   ...
   if(kilp->Maali(osuus, 0) != TMAALI0 && osuushyv(kilp, osuus) &&
	  ...
	  switch(kilp->wTark(osuus, 1)) {
		 ...
		 case L'E' : wcscpy(oas, L"Ei l�ht.  "); break;
...
//#else   // T�st� alkaa HALIKKO tai MKV
...
static int prttulos_nt(kilptietue *kilp, tulostusparamtp *tulprm, int osuus, int sj)
   ...
   INT32 tls, tls1;
   FldFrmtTp *fld;
   wchar_t *txts[MAXLANG][10] = {{L"Hyl.", L"Kesk.", L"Ei l�ht.", L"Virhe", L"Puuttuu", L"Sakko", L"Tasoitus", L"Ohitettu", L"Poissa", L"V�liajat:"},
   ...
   if(kilp->Maali(oluku-1, 0) != TMAALI0 && !joukkuekh(kilp)) {
	  ...
	  if (tulprm->language == 0) {
		  switch(kilp->wTark(-1, 1)) {
			 ...
			 case L'H' : wcscpy(as, L"HYL�TTY   "); break;
			 case L'E' : wcscpy(as, L"EI L�HT.  "); break;
...
//         tarkcom();
      ...
      vidspmsg(ySize-3,14,7,0,".os.   Paina <Esc> keskeytt��ksesi");
      sccurset(ySize-3,49);
      }
   return(ch);
   }
	...
	//						nhyl[*srj][tulprm->osuus] - neil[*srj][tulprm->osuus];
					if (!ei_lukum && navoin[*srj][tulprm->osuus] != nilm[*srj]) {
						...
						if (tulprm->language > 0) {
							...
							swprintf(wline,L"<p>L�hti : %d,   Keskeytti : %d,"
								L"   Hyl�tty : %d",ntulos[*srj][los][tulprm->piste]+
								nkesk[*srj][los]+nhyl[*srj][los],nkesktot,nhyltot);
							if (navoin[*srj][tulprm->osuus]) {
				...
				if (tulprm->monios)
					...
					if (tulprm->language > 0) {
						...
						if (!tulprm->piste)
							...
							swprintf(wline+wcslen(wline),L"<p>%s - %d. osuus - %d. v�liaika",
								snimi,tulprm->osuus+1,tulprm->piste);
						}
					}
					...
					if (!ei_lukum && navoin[*srj][tulprm->osuus] != nilm[*srj]) {
						if (navoin[*srj][tulprm->osuus] != 0 ) {
							...
							if (tulprm->language > 0)
								...
								swprintf(wline,L"<p>L�hti : %d   Keskeytti : %d   Hyl�tty : %d\n",
									ntulos[*srj][tulprm->osuus][tulprm->piste]+nkesk[*srj][tulprm->osuus]+
									nhyl[*srj][tulprm->osuus],nkesk[*srj][tulprm->osuus],nhyl[*srj][tulprm->osuus]);
							}
						tulprm->writehtml(wline);
	...
	if (tulprm->viimos == L'K' || tulprm->monios)
		...
		if (tulprm->language > 0) {
			...
			if (!tulprm->piste)
				...
				swprintf(wline,L"%-10s  %d. osuus, %d. v�liaika",snimi,tulprm->osuus+1,tulprm->piste);
			}
		}
	putfld(tulprm, wline, 0, wcslen(wline), 0, 0);
	endline(tulprm, 1);
	...
	//					nhyl[*srj][tulprm->osuus] - neil[*srj][tulprm->osuus];
				if (!ei_lukum && navoin[*srj][tulprm->osuus] != nilm[*srj]) {
					...
					if (tulprm->language > 0) {
						...
						swprintf(wline,L"L�hti : %-4d   Keskeytti : %-3d"
							L"   Hyl�tty : %-4d",ntulos[*srj][los][tulprm->piste]+
							nkesk[*srj][los]+nhyl[*srj][los],nkesktot,nhyltot);
						if (navoin[*srj][tulprm->osuus]) {
							wcscat(wline, L"   Avoinna : ");
							_itow(navoin[*srj][tulprm->osuus], wline+wcslen(wline), 10);
				...
				if (tulprm->monios)
					...
					if (tulprm->language > 0) {
						...
						if (!tulprm->piste)
							...
							swprintf(wline,L"%-10s  %d. osuus, %d. v�liaika",
								snimi,tulprm->osuus+1,tulprm->piste);
						}
					}
					...
					if (!ei_lukum && navoin[*srj][tulprm->osuus] != nilm[*srj]) {
						if( navoin[*srj][tulprm->osuus] != 0 ) {
							...
							if (tulprm->language > 0)
								...
								swprintf(wline,L"L�hti : %d   Keskeytti : %d   Hyl�tty : %d",
									ntulos[*srj][tulprm->osuus][tulprm->piste]+nkesk[*srj][tulprm->osuus]+
									nhyl[*srj][tulprm->osuus],nkesk[*srj][tulprm->osuus],nhyl[*srj][tulprm->osuus]);
							}
						putfld(tulprm, wline,0,60,0, 0);
...
//	else
	...
	if (n > 0) {
		...
		if (tulprm->kohde == L'P') {
			...
			if (!autotl)
				vidspmsg(ySize-1,30,7,0,"Paina <Esc> keskeytt��ksesi");
			tulfl =TRUE;
			tcomfl = FALSE;
			}
...
void yhteenveto(tulostusparamtp *tulprm)
   ...
   ots = L" Sarja      Ilm    Aika   Kesk    Hyl   Auki  K�rkiaika  Emit  Yht.l�ht�";
#else
#ifdef LUENTA
...
*/
	  ...
	  if (sarjaluku > 1) {
		 swprintf(wline,L"  Yhteens�%5d",nilmt);
		 newline(tulprm, 1);
		 putfld(tulprm, wline, 0, 80, 0, 0);
		 endline(tulprm, 0);
...
//		   aftulparam.yksihtml = ch == L'Y';
		   ...
		   wselectopt(L"N�ytet��nk� kilpailunumerot (K/E)", L"KE", &ch);
...
void joukkuepisteval(tulostusparamtp *tulprm)
   ...
   do {
	  wselectopt(L"T)ulosta pisteet, L)ue m��rittelyt, M)��rittele, Esc: peruuta",
		 L"TLM\x1b", &ch);
	  switch (ch) {
		 case L'L' :
			luePisteSrj(0);
			clrln(ySize-3);
			vidspwmsg(ySize-3,0,7,0,L"Paina jotain n�pp�int�");
	  ...
	  if (kilp.kilpstatus == 0 && kilp.id() != 0) {
		 ...
		 if ((!p || v > 0) && seura[0]) {
			if (ns < MAXNSEURA && (p = (wchar_t *) calloc(lseura+1, 2)) != NULL) {
			   ...
			   writeerror_w(L"Ei tilaa t�ydelliselle seuraluettelolle", 0);
			   break;
			   }
			}
		 }
...
void joukkuetul(tulostusparamtp *tulprm)
   ...
   int sijapisteet[1000];
	...
	if (k_pv > 0) {
		ch = ytfl ? L'Y' : L'P';
		wselectopt(L"Y)hteistuloksista, P)�iv�n tuloksista", L"YP", &ch);
		ytfl = ch == L'Y';
		}
	while (1) {
		clrln(ySize-3);
		vidspwmsg(ySize-3,0,7,0,L"       Anna sarja :               "
			L"<F4>:Yhdistelm�, <Esc>:Poistu");
		sarjayhdfl = 0;
		srj = luesarja(L"", &tc);
		if (tc == 204) {
			srj = sarjayhdistelma();
		...
		wselectopt(L"U)rheiluseurat, muut Y)hteis�t, K)aikki", L"UYK", &srlj);
#if JKOKO > 3
		ch = L'0' + jkoko;
		...
		wselectopt(L"A)ikojen / S)ijoitusten mukainen j�rjestys", L"AS", &ch);
		sijaluvut = (ch == 'S');
...
void joukkuetul(tulostusparamtp *tulprm)
   ...
   if (pm) {
      ...
      if (n > PM_LUKU) {
		 ...
		 writeerror_w(L"Lukum��r� pienennetty maksimiarvoonsa 8", 0, true);
         }
      clrln(ySize-3);
	  }
   else
		...
		if ((d = getpos(kno)) > 0) {
			...
			for (os = 0; os < Sarjat[kilp.sarja].osuusluku; os++) {
				...
				if (kh == L'E' && tl != TMAALI0) {
					swprintf(line, L"%4d-%d\t%s\t%s\t%s\t%s\n", kno, os+1, kilp.Joukkue(st1, 40),
						kilp.Maa(), kilp.Nimi(st, OSNIMIL, os), L"Tulos ei-l�hteneess� joukkueessa");
					OutFl->WriteLine(line);
					}
				...
				if (os > 0 && tl != TMAALI0 && kilp.Maali(os-1, 0) == TMAALI0 &&
					...
					if (kilp.ostiet[os].ylahto == TMAALI0)
						swprintf(line, L"%4d-%d\t%s\t%s\t%s\t%s\n", kno, os+1, kilp.Joukkue(st1, 40),
							kilp.Maa(), kilp.Nimi(st, OSNIMIL, os), L"L�ht�aika avoinna");
					OutFl->WriteLine(line);
					}
				}
			}
...
void metsassa(tulostusparamtp *tulprm)
{
	...
	ch = L' ';
	wselectopt(L"K)aikki, E)nnen aikavalintaa l�hteneet", L"KE", &ch);
	clrln(ySize-3);
	if (ch == L'E') {
#ifdef _CONSOLE
		vidspwmsg(ySize-3, 0, 7, 0, L"L�ht�ajan yl�raja:");
		...
		switch (tulprm.tulostettava) {
			...
			case L'V':
				...
				if (tulprm.piste == 0) {
					...
					tulprm.rajaus = L'T';
				break;
			case L'Y':    // Yhdistelm� vaihto+v�liaikoja
				tulprm.viimos = L'K';
				tulprm.kaikkiva = true;
				tulprm.kaikkisijat = ((options & 8) != 0);
				break;
