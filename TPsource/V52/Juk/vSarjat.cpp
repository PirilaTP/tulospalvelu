...
// Viestikilpailun sarja- ja kategoriatietojen hallinta sekä lähtöaikojen laskenta.
...
int maaraaOsuusluku(bool kysy)
{
	...
	if (!kysy) {
		...
		if (er) {
			wchar_t ch = L' ';
			wselectopt(L"Osuuslukum��riss� ristiriita. Yritet��nk� lukea tiedot? (K/E)", L"KE", &ch);
			if (ch == L'E')
				lopetus();
			}
		if (kilpparam.n_os_akt < kilpparam.osuusluku) {
...
error :
		...
		strcpy(prs, "Sarjatietoja ei l�ydy, k�ytt� mahdotonta");
		writeerror(prs, 0);
		scpclr();
		return(1);
		}
