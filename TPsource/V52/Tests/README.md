...
tai VCL:n ja lukevat globaaleja (`Sarjat[]`, `t0`, `kilpparam`). Niitä ei voi
linkittää testeihin vetämättä mukaan koko ohjelmaa ja tietokantaa.

Käytetty tapa on erottaa päätöslogiikka omaan pieneen käännösyksikköönsä, joka
saa kaiken tarvitsemansa parametreina:

- `Juk/VOtsikot.cpp` sisältää logiikan, ei riipu globaaleista eikä Windowsista
- `Juk/VTulostus.cpp` sisältää ohuen adapterin, joka poimii globaalit ja kutsuu
  edellistä
...
Muu projekti on ISO-8859-1:tä, mutta `Tests/`-hakemiston ja `Juk/VOtsikot.*`:n
lähdetiedostot ovat **pelkkää ASCIIta**: kommenteissa ei käytetä skandeja ja
merkkijonoliteraaleissa ne kirjoitetaan `\u`-escapeina, esim. `L"L\u00e4ht\u00f6: "`.

Näin samat tiedostot kääntyvät MSVC:llä, g++:lla ja C++Builderilla ilman
merkisto-optioita, ja doctestin tulosteet näkyvät oikein sekä Windowsin
