...
void merkisto(wchar_t merkit)
   ...
   switch (merkit) {
	  ...
	  case L'P' : 
				...
				delete mf;
				 pscr = 1;
	  case L'W' :                    // my�s 'P' jatkuu
	  case L'G' :                    // my�s 'P' jatkuu
				 utrlate['�'] = 196;
				 utrlate['�'] = 214;
				 utrlate['�'] = 197;
				 utrlate['�'] = 220;
...
static int luevaihevalinta(void)
{
	...
	if (cfgfile->IsOpen()) {
		while (!cfgfile->Feof()) {
			...
			if ((p = wcstok(line, L"= \n", &ctx)) != NULL) {
				if (wcscmpU(p, L"VAIHE") == 0 || wcscmpU(p, L"P�IV�") == 0) {
					if ((p = wcstok(NULL, L"= \n", &ctx)) != NULL) {
						i_pv = _wtoi(p) - 1;
						}
					break;
...
static void getipparam(INT ny, wchar_t *str)
 ...
 static void gettauluipparam(int ny, wchar_t *str)
	...
	if (ny < MAX_PORTTI) {
		...
		p = str;
		if (wcswcind(*str, L":,/=") >= 0 && (p = wcstok(str+1, L":", &ctx)) != NULL) {
			wcsncpy(ipparam[ny].destaddr, p, 63);
			if ((p = wcstok(NULL, L":", &ctx)) != NULL && _wtoi(p) > 100)
...
/*
		 ...
		 if(!wmemcmpU(fldn, L"NOLLAK�RKI", 10)) {
			tulosmuot.nollakarki = 1;
			continue;
			}
		 if (!wmemcmpU(fldn, L"P�IVI�=", 7)) {
			kilpparam.n_pv_akt = fldn[7] - L'0';
			continue;
			}
		 if (!wmemcmpU(fldn, L"V�LIAIKOJA=", 11)) {
			kilpparam.valuku = fldn[11] - L'0';
			continue;
			}
*/
		 if (!wmemcmpU(fldn, L"P�IV�=", 6) || !wmemcmpU(fldn, L"VAIHE=", 6)) {
			 if (fldn[6] == L'V')
				 vaihevalinta = 1;
			 else {
				 k_pv = fldn[6] - L'1';
...
*/
			 ...
			 if(!wmemcmpU(fldn, L"KUNTOL�HT�", 8)) {
				KuntoMaali = -1;
				continue;
				}
			 ...
			 if(!wmemcmpU(fldn, L"LIS�VAKANTTIALKU=", 7)) {
				if ((p = wcsstr(fldn, L"=")) != NULL) {
			...
			if(!wmemcmpU(fldn, L"LIS��EDELLE", 8)) {
				lisaa_edelle = 1;
				continue;
			}
...
*/
		if(!wmemcmpU(fldn, L"VAADITARK", 8)) {
		 ...
		 if(!wmemcmpU(fldn, L"L�HET�CR=EI", 10)) {
			lahetacr = 0;
			continue;
			}
...
/*
		 ...
		 if(!wmemcmpU(fldn, L"HPTARRA=", 8)) {
			...
			k = 1;
			rtarra = 1;
			if ((p = wcstok(fldn+8, L"/,", &ctx)) != 0)
			   rtarra = _wtoi(p);
			if ((p = wcstok(NULL, L"/,", &ctx)) != 0)
			   k = _wtoi(p);
			ntarra = k*rtarra;
			if ((p = wcstok(NULL, L"/,", &ctx)) != 0)
			   ftarra = _wtoi(p);
...
*/
		 ...
		 if(!wmemcmpU(fldn, L"EST�MUUTOS", 8)) {
			esta_tall = 1;
			continue;
				}
#ifdef AUTOFILE
		 if (!wmemcmpU(fldn, L"HTML=", 5) || !wmemcmpU(fldn, L"MHTML=", 6) || !wmemcmpU(fldn, L"XML=", 4)) {
			...
			if (p && *p != L'M' && (*p == L'S' || ((p = wcstok(NULL, L",/", &ctx)) != NULL) && *p == L'S')) {
			   aftulparam.yksihtml = 0;
			   autofileparam.vainMuutt = true;
			   }
			if (p && (*p == L'M' || ((p = wcstok(NULL, L",/", &ctx)) != NULL) && *p == L'M')) {
		 ...
		 if( !wmemcmpU(fldn, L"YHTEISL�HT�",7)) {
			yhtlahto = 1;
            continue;
            }
		 ...
		 if (!wmemcmpU(fldn, L"MONITORV�LI=",12)) {
...
//			   getipparam(MAX_PORTTI, fldn+pos+4);
		...
		if (!wmemcmpU(fldn, L"MAXYHTEYS=",10)) {
		 ...
		 if (!wmemcmpU(fldn, L"V�LIT�",6) && (pos = 6) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
				if (pos == 6) {
		...
		if( !wmemcmpU(fldn, L"��NI=",5)) {
			aani = fldn[5] - L'0';
			continue;
			}
		 if( !wmemcmpU(fldn, L"LIS��=",6)) {
...
//			 }
         ...
         if( !wmemcmpU(fldn, L"L�HT�RYHM�=",11)) {
...
#endif
		 if( !wmemcmpU(fldn, L"L�HKELLO",8) && (pos = 8) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
		 ...
		 if( !wmemcmpU(fldn, L"L�HAIKA",7) && (pos = 7) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
			ajanottofl = 1;
			com_ajat[ny-1] = 1;
			if (fldn[pos+1] == L'V')
		 ...
		 if( !wmemcmpU(fldn, L"L�HECODE",8) && (pos = 8) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
			lahec[ny-1] = 1;
			if (fldn[pos+1] == L'V')
				lahec[ny-1] = 2;
		 ...
		 if( !wmemcmpU(fldn, L"L�HTIEDOSTOT",8)) {
			y = -1;
		...
		if (!wmemcmpU(fldn, L"EST�NEG",7)) {
			estaneg = 1;
			continue;
			}
		if (!wmemcmpU(fldn, L"EST�HAAMUT",7)) {
			estaneg = 1;
			if (fldn[10] == L'=') {
...
//			   regnly_no[0] = 0;
...
#endif
		 if (!wmemcmpU(fldn, L"L�HT�IKKUNA=", 12)) {
		 ...
		 if( !wmemcmpU(fldn, L"L�HDEPISTEET",8)) {
			lahdepistehaku = 1;
			continue;
			}
         ...
         if( !wmemcmpU(fldn, L"VAINV�LIAJAT",12)) {
			vainvaliajat = 1;
			continue;
            }
...
//#if defined(UINTI) || defined(MERI5)
		 if( !wmemcmpU(fldn, L"TALL-L�HD",6)) {
            era_lahdot = 1;
            continue;
			}
//#endif
...
////			sak_com = _wtoi(fldn+10);
		 ...
		 if( !wmemcmpU(fldn, L"EIL�HT=AVOIN", 8)) {
			eilavoin = TRUE;
			continue;
			}
		 ...
		 if( !wmemcmpU(fldn, L"ELINKL�HT�",7)) {
		 ...
		 if( !wmemcmpU(fldn, L"L�HT�VERT=",10)) {
...
//               ny = fldn[6] - L'1';
			...
			if (!wmemcmpU(fldn, L"EMITAGLIS�=0", 12)) {
				emitaglisa = 0;
				continue;
				}
			if (!wmemcmpU(fldn, L"EMITAGLIS�", 9)) {
				emitaglisa = 1;
				continue;
				}
			...
			if (!wmemcmpU(fldn, L"ETHAKUV�LI=",11)) {
...
//					wmemmove(eTdate+6, eTdate+7, 3);
		 ...
		 if (!wmemcmpU(fldn, L"HTMLS�ILNIMI",9)) {
            html_sail_nimi = 1;
            continue;
			}
         ...
         if (!wmemcmpU(fldn, L"EST�EMITTOISTO",10)) {
		...
		if( !wmemcmpU(fldn, L"SJBOX",5) || !wmemcmpU(fldn, L"L�HDEPISTEET",8)) {
			lahdepistehaku = 1;
			continue;
			}
		...
		if( !wmemcmpU(fldn, L"ANNASEUR",8)) {
         ...
         if (!wmemcmpU(fldn, L"VALON�YTT�=",11)) {
		 ...
		 if( !wmemcmpU(fldn, L"L�HEMVA",7) && (pos = 7) != 0 &&
			(ny = yhteys_no(fldn, &pos)) != 0) {
            emitfl = 1;
            com_emitva[ny-1] = 1;
            continue;
...
void lueeratiedot(wchar_t *fname)
   ...
   if (erafl != NULL && erafl->IsOpen()) {
	  ...
	  writeerror_w(L"Tiedostoa ERADAT.LST ei l�ydy", 0, true);
   if (erafl != NULL)
		delete erafl;
   }
//#endif
...
korjval:
   ...
   if (ch) {
		wselectopt(L"Merkist�: I)BM, A)scii-7, R)oman-8, P)ostscrpt"
			L", W)indows,L)evylt�", L"ARLIPWG",&merkit);
		if (merkit == L'W') {
			ch2 = L' ';
			wselectopt(L"K�ytet��nk� Windowsin kirjoitinajureita (K/E)", L"KE", &ch2);
			if (ch2 == L'K')
				merkit = L'G';
			}
		}
   clrln(ySize-3);
   switch (merkit) {
      case L'A' : viwrrectw(*y,63,*y,78,L"Merkist� Ascii-7",7,0,0);
                 break;
      case L'R' : viwrrectw(*y,63,*y,78,L"Merkist� Roman-8",7,0,0);
                 break;
      case L'P' : viwrrectw(*y,62,*y,79,L"Merkist� Postscript",7,0,0);
                 break;
	  case L'W' : viwrrectw(*y,63,*y,78,L"Merkist� Windows",7,0,0);
                 break;
      case L'L' : viwrrectw(*y,63,*y,78,L"Merkist� levylt�",7,0,0);
                 break;
#ifdef WIN32
      case L'G' : viwrrectw(*y,63,*y,78,L"Windowskirjoitin",7,0,0);
                 break;
#endif
      default  : viwrrectw(*y,63,*y,79,L"Merkist� IBM (PC)",7,0,0);
      }
   (*y)++;
   if (ch) {
      switch (aani) {
      ...
      wselectopt(L"��nimerkki: E)i k�ytet�,  vain V)irheist�,  my�s H)uomio��ni"
         ,L"EVH", &ch2);
      switch (ch2) {
   ...
   viwrrectw(*y,5,*y,16,L"��nimerkki :",7,0,0);
   switch (aani) {
      case 0  : viwrrectw(*y,18,*y,28,L"Ei k�yt�ss�", 7, 0, 0); break;
      case 1  : viwrrectw(*y,18,*y,31,L"Vain virheist�", 7, 0, 0); break;
      default : viwrrectw(*y,18,*y,39,L"Virhe- ja huomiomerkit", 7, 0, 0);
      }
   ++*y;
#ifndef COMOFF
	wcscpy(msg, L"K�yt�ss� yhteydet:");
...
//	GetVersionEx(&osvi);
   ...
   if (k_pv == -1) {
	...
	if (minvakanttiluku > n_lisaa) {
		writeerror_w(L"Parametrin VAKANTTILUKU arvo liian suuri. Parametri LIS�� ehk� syyn�.", 0);
		return(1);
		}
   ...
   for (i = 0; i < 9; i++) if (!maalit[i]) maaliajat[i] = TMAALI0;
	...
	if (ToimintaTila == 2) {
		...
		if (lajatfl && !pvparam[k_pv].hiihtolahto) {
			writeerror_w(L"Parametri LAJAT, mutta l�ht�portin aikojen k�ytt�� ei valittuna", 0);
			}
