...
// XML-tulostiedoston kirjoitus IOF 2.0 -muodossa henkilökilpailun tuloksista.
...
int xmlots(tulostusparamtp *tulprm)
   ...
   swprintf(ln+wcslen(ln), L" - p�iv� %d", k_pv+1);
...
int xmlsanoma(char *buf, int maxlen, kilptietue& kilp, int dkilp, int piste, int msgno)
   ...
   if (kilpparam.n_pv_akt > 1) {
		swprintf(str+wcslen(str), L" - p�iv� %d", k_pv+1);
...
int xmlfissrjots(int sarja, tulostusparamtp *tulprm)
   ...
   tul_tied = tulprm->lstf->u.Fl;
	...
	if (wcswcind(towupper(Sarjat[sarja].sarjanimi[0]), L"NDLWT") >= 0)
		...
		writeerror_w(L"Sukupuolta ei voi p��tell� sarjan nimest�. Vaihda sarjan nimi", 0);
		return(1);
		}
   ...
   clrln(ySize-3);
#ifdef _CONSOLE
   vidspwmsg(ySize-3, 0, 7, 0, L"Kilpailun m��ritystiedosto:");
   while (1) {
...
#ifdef _CONSOLE
	   writeerror_w(L"Tiedostoa ei l�ydy, peruuta painamalla Enter ja ESC", 0);
#else
	   writeerror_w(L"Kilpailun m��ritystiedostoa ei ole valittu", 0, true);
	   return(1);
#endif
	   }
   if (kisa_tied)
