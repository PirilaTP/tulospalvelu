...
// Joukkue- ja osuustietueiden alustus sekä nollaustoiminnot viestikilpailussa.
...
INT32 kilptietue::tTulos(int tosuus, int va, __int64 *tlslisa /* = NULL */)
{
   ...
   // K�yd��n l�pi tosuus ja aiemmat osuudet ja
...
INT32 kilptietue::Tulos(int osuus, int va, bool sekvajaat /* = false */)
{
	...
	if ( osuus >= 0 && Maali(osuus, va) != TMAALI0 &&
		...
		for (os = osuus; os >= 0; os--) {          // os == 0 lis�tty 8.3.2010
		...
		else {                //  yhteisl�ht� tai partio
			if (kilpparam.partio || osuus == 0) {            // osuus == 0 lis�tty 8.3.2010
