...
// Konsoliohjelman main-funktio, käynnistys, lopetus ja Ctrl+C-käsittelijä.
...
void valikko(void)
   ...
   extern scr paavalikko;
#ifdef MAKI
	if (k_pv == makipv)
		paavalikko.l[1].t = L"M)�ki           M�kikilpailun tulospalvelu";
#endif
	if (bootfl & 8) {
		int jj = (bootfl / 16) & 15;
		if (jj && jj <= maxjono)
	...
	do {
      ...
      if (bootfl < 2) {
			if (k_pv == makipv)
				wselectopt(L"M)�ki, K)orjaa, S)elost., T)ulos, taU)lu, A)setus, Y)hteys, "
      ...
      switch(ch) {
		...
		if (ch == L'P') {
         ch = L' ';
#if LANG == SV
         wselectopt(L"Sluta och st�nga av? (J/N)",
            L"JN", &ch);
         if (ch == L'J') ch = L'P';
#else
