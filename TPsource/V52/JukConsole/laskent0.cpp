...
// Viestikilpailun päävalikko ja käyttöliittymän perustoiminnot konsolikäytössä.
...
void copydfile(datafile *datfb, int oldfile)
   ...
   HANDLE thandle;
	...
	if ((buf = (char *) malloc(CBUFL)) == 0) {
      writeerror("Muisti ei riit� kopioinnille", 0);
      return;
      }
	memset(buf, 0, CBUFL);
...
void asetukset()
   ...
   kehys(1);
   prscr(&asetusvalikko);
   selectopt("T)arrat, A)jat, V)ar.-kynnys, L)�hd�t, U)DP, D)os, loK)i, "
   ...
   switch (ch) {
	  case 'C' :
		  ...
		  break;
      case 'T' : ch = ' ';
                 selectopt("K)eskeyt�, L)�het� toiselle PC:lle, T)ulosta"
                    ", E)i-l�hteneet","KLTE", &ch);
                 ...
                 switch (ch) {
                    ...
                    case 'L' :
                       if (comfl && (lahcom[0] || lahcom[1] ||
                          ...
                          tarfl = FALSE;
                          clrln(ySize-3);
                          viwrrect(ySize-3,0,ySize-3,39,"Luettele tarral�hetyksen "
                             "tiedonsiirrot: ",7,0,0);
						  p = msg;
					   ...
					   else writeerror("Tiedonsiirto ei k�yt�ss�",0);
					   break;
					case 'T' :
					   break;
                    case 'E' :
                       ch = ' ';
                       selectopt("T)arrat ei-l�hteneille,   "
                          "E)i tarroja", "TE", &ch);
                       eiltarra = (ch == 'T');
                       break;
                    }
      ...
      case 'V' : for (r = 2; r < ySize-4; r++) clrtxt(r,0,64);
                 if ( (varfhandle = _open(varfname, O_RDWR | O_BINARY |
                    ...
                    if (_filelength(varfhandle) > 0) {
                       ...
                       ch = ' ';
                       selectopt("Luetaanko aiemmat kynnykset levylt� (K/E)"
                          , "KE", &ch);
      ...
      case 'L' :
         ch = ' ';
         selectopt("L)�ht�ajat vai Y)hteisl�ht�     Esc  : Peruuta",
            "LY\x1B", &ch);
         switch (ch) {
            case 'L' :
               if (sarjaluku == 1) {
                  clrln(ySize-3);
                  viwrrect(ySize-3,0,ySize-3,16,"Anna l�ht�aika : ",7,0,0);
                  tl[0] = Sarjat[0].lahto;
                  ...
                  vidspmsg(ySize-3,0,7,0, "\x19\x18 : Siirry, + : Hyv�ksy "
            ...
            case 'Y' :
               ...
               do {
                  ...
                  vidspmsg(2,0,7,0,
                     "Sarja     Yhteisl�ht� Vaihto kiinni");
                  ylmsg =

                     "Yhteisl�ht� merkit��n au-"
                     "tomaattisesti, jos vaih- "
                     "don sulkemisaika annettu "
                     "Kumoa 'Y' saman n�pp�imen"
                     "toistolla.               "
                     "                         "
                     " P : Poista sulkemisaika "
                     " C : Kopioi ensimm�isen  "
                     "     sarjan tiedot kai-  "
                     "     kille sarjoille     "
                  ...
                  viwrrect(19,60,19,64,ylm ? "Ei   " : "Kyll�",7,0,0);
                  paivita:
      ...
      case 'A' : 
		  ...
		  switch (regnly[0]) {
				case 1:
				case 2:
					selectopt("L)�hetys, A)jantarkkuus, M)aalikellon aika", "ALM", &ch);
					break;
				default:
					selectopt("L)�hetys, A)jantarkkuus, M)aalikello, S)irit", "ALMS", &ch);
					break;
				}
			switch (ch) {
				 case 'S':
#ifdef LAJUNEN
					 ch = ' ';
					 selectopt("K�ynnist� yhteydet uudelleen (K/E)", "KE", &ch);
					 if (ch == 'K')
						 reconnectSirit[0] = 1;
#endif
					 break;
				 case 'L':
					   ...
					   if (tc != ESC) {
						  ch = ' ';
						  selectopt("Vahvista l�hetys (K/E)", "KE", &ch);
					...
					case 'A':
					   ch = ' ';
					   selectopt("N�yt� sekunnin kymmenykset (K/E)", "KE", &ch);
