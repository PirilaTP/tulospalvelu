...
// Konsolipohjainen näyttö- ja syöttörutiinit: aikajonojen ja joukkuetietojen näyttö viestikilpailussa.
...
void seur_mtr(void)
   ...
   if (emitfiletest) {
	  ch = ' ';
	  selectopt("Lukemisajat T)iedostosta EMIT_IN.DAT vai V)akiov�lein", "TV", &ch);
	  if (ch == 'V') {
		 clrln(ySize-3);
   ...
   INT mm = 0, sek = 0, osat = 0;
	...
	if (!ed_em[r_no]) {
	  if ((ed_em[r_no] = (emittp *) malloc(sizeof(emittp))) == 0) {
		 writeerror("Muisti ei riit� Emit-tiedoille", 0);
		 return(1);
		 }
	  }
   ...
   if (jalkisyotto) {
      ...
      do {
         ...
         if (ch == '%') {
			...
			sprintf(msg, "Joukkue %d, osuus %d,  H)yv�ksy, M)uuta, Esc: Peruuta", kno, os);
            ch = 'H';
		 ...
		 if (esitark >= 0 && os >= 0 && EmitJarr(kno, os) != -1
			...
			if (kaikki_ajat[0] || emitfiletest) {
               ...
               if (ch == '%') {
                  ...
                  sprintf(msg, "Joukkue %d, osuus %d,  H)yv�ksy, M)uuta, Esc: Peruuta", j, i );
                  clrln(ySize-3);
                  ch = 'H';
                  selectopt(msg, "HM\x1b", &ch);
				  if (ch == 'H') {
   ...
   if (em.badge && !bdg2kno(em.badge, 0) && !emitfiletest) {
	  ...
	  do {
         ...
         if (ch == '%') {
			...
			sprintf(msg, "Joukkue %d, osuus %d,  H)yv�ksy, M)uuta, Esc: Peruuta", os, kno );
            clrln(ySize-3);
            ch = 'H';
			selectopt(msg, "HM\x1b", &ch);
            if (ch == 'H')
         ...
         switch (ch) {
			...
			case 'H':
               do {
                  ...
                  kno = kilp.kilpno;
				  sprintf(msg, "No %d, osuus %d.  H)yv�ksy valinta, U)usi "
   ...
   if (ch == 'O' || (em.package >= 20000000L && !em.badge)) {
	...
	if (!em.badge) {
		...
		if ((em.kilpno = bdg2kno(em.badge, 0)) < minkilpno || (d = getpos(em.kilpno)) <= 0) {
			sprintf(msg, "Korttia %ld vastaavaa kilpailijaa ei l�ytynyt",
				em.badge);
			em.kilpno = 0;
			writeerror(msg, 0);
			}
		while (jatka && d > 0 && esitark >= 0) {
			...
			if ((kh = tark_badge(&em, &kilp, false)) < 0 && !emitfiletest) {
				naytatulos(3);
				sprintf(msg, "Korttia %ld vastaavaa rataa ei l�ytynyt",
					em.badge);
				writeerror(msg, 0);
				}
			tallfl = kh > 0;
			...
			if (rt && rt->maalilaji == 4 && em.maali == TMAALI0/KSEK) {
				...
				if (em.maali == TMAALI0/KSEK) {
					if (!emitfiletest) {
						naytatulos(3);
						writeerror("Maaliaikaa ei pystytty m��r��m��n", 0);
						}
					}
				else
					tallkilp = 1;
...
static int haejoustoosuus(kilptietue *kilp, aikatp ut)
{
	...
	// Hae ensimm�inen osuus, jolla ei aikaa
	for (osuus = Sarjat[kilp->sarja].osuusluku -2; osuus >= 0 && kilp->Maali(osuus, 0) == TMAALI0; osuus--);
	osuus++;

	// Jos edellinen aika poikkeaa v�emm�n kuin (ylimraja+1sek), valitse edellinen osuus
	if (osuus > 0 && NORMKELLO(purajak(ut.t) - kilp->Maali(osuus-1, 0)) < ylimraja+SEK) {
		osuus--;
		}
	// Muussa tapauksessa valitse ensimm�inen ilman aikaa ollut osuus ja etsi osuuksien lopusta alkaen
	// badge ja poimi sielt� osanottajan nimi
...
#ifndef LANGSV_K
			writeerror("AIKATAULUKKO T�YSI",0);
#else // LANGSV_K
		...
		if (vastaus->r1.tunnus >= 'B' && vastaus->r1.tunnus <= 'D') {        // RTR-nollaustieto
			...
			if (ylimraja) {
				...
				for (int os = 0; os < Sarjat[kilp.sarja].osuusluku; os++) {
					...
					// Etsi aikaa, joka poikkeaa v�hemm�n kuin ylimraja
					for (int va = 0; va <= Sarjat[kilp.sarja].valuku[os]; va++) {
						// Katso ensin, l�ytyyk� eroaako aika v�hemm�n jostain kilpailijan (v�li)ajasta
						if ((ttm = kilp.Maali(os, va)) != TMAALI0 &&
							abs((int) NORMTULOS(purajak(ut.t) - ttm)) < ylimraja) {
							aikatp vertt;
							// Jos t�llainen piste l�ytyi, etsi ajanottojonosta lopusta alkaen
							// rivi�, jolla sama kilpailija ja piste. Jos on aseta ut.status = 2
...
//					return(0);
			...
			if (ut.kno > 0 && ut.piste >= 0 && osat < Sarjat[kilp.sarja].osuusluku &&
				...
				if (kilp.Tark(osat, 0) == 'E') {
...
#endif
					if (loki)
						kirjloki("OSUUS ON JO MERKITTY EI L�HTENEEKSI\r\n");
					EnterCriticalSection(&tall_CriticalSection);
					}
					...
					LeaveCriticalSection(&tall_CriticalSection);
#ifdef _CONSOLE
					vidspmsg(ySize - 4, 0, 0, 7, "Joukkueella      poikkeavan hyv� osuusaika");
					vidint(ySize - 4, 12, 4, kno);
#endif
					if (loki) {
						kirjloki("POIKKEAVAN HYV� OSUUSAIKA\r\n");
						}
					EnterCriticalSection(&tall_CriticalSection);
					}
				}
...
void tall_ec(UINT32 bdg, INT valeim, INT32 aika, INT kielto)
{
	...
	if ((kno = bdg2kno(bdg, 0)) > 0 || (kilpparam.kaksibadge && (toinen = 1) != 0 && (kno = bdg2kno(bdg, &toinen)) > 0)) {
		...
		if ((!vaikavert || t < 600*vaikavert) && (d = getpos(kno)) > 0) {
			...
			sprintf(msg, "Yli %d min aikaero saapuneen aikatiedon ja t�m�n koneen v�lill�", vaikavert);
			msg[80] = 0;
			writeerror(msg, 4000);
			}
		}
...
void vahaku(LPVOID lpCn)
{
  ...
  if ((er = httphaku(eTParam.eThost , eTParam.eTport, page, 0, eTParam.buf, eTParam.buflen, &eTParam.haettu)) != 0) {
	  swprintf(msg, L"Virhe %d v�liaikahaussa", er);
   ...
   if ( (aikajono[ino]->afhandle = _wopen(aikajono[ino]->aikafname, O_RDWR | O_BINARY | O_CREAT,
	  ...
	  if (fl >= 10) {
		 ...
		 if (sail_ajat)
			...
			ch = L' ';
#ifndef LANGSV_K
			wselectopt(L"AJANOTTOTOIMINNON K�YNNISTYS - Luetaanko aiemmat "
			   L"tiedot levylt� (K/E)", L"KE", &ch);
#else
			wselectopt(L"STARTEN AV FUNKTIONEN M�L - Skall tidigare data  "
			   L"l�sas fr�n fil (J/E)", L"JE", &ch);
			wtransl(L"J", L"K", &ch);
#endif
#else
			int i;
...
#ifndef LANGSV_K
			 vidspmsg(ySize-3,0,7,0,"Paina ajanottoon varattavaa n�pp�int�");
#else // LANGSV_K
			 vidspmsg(ySize-3,0,7,0,"Tryck tangenten f�r tidtagningen     ");
#endif // LANGSV_K
			 i = nivkeys;
			 nivkeys = 0;
			 ch = readkbd(&key, 1, 0);
...
#ifndef LANGSV_K
				writeerror("Ajanotton�pp�in ei voi olla 'Enter', 'Esc', numero tai kirjain",0);
#else // LANGSV_K
...
#ifndef LANGSV_K
			 sprintf(prs,"N�pp�inkoodi (%d,%d) %s varattu ajanotolle, vahvista "
				"(K/E)", (int) ch, (int) key, st);
			 tc = ' ';
			 selectopt(prs, "KE", &tc);
#else // LANGSV_K
			 sprintf(prs,"Tangentkod (%d,%d) %s reserverats f�r tidtagningen, bekr�fta "
