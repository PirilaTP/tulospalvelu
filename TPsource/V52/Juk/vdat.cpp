...
// Kenttärakenteiden ja tietuekenttämääritysten alustus viestikilpailun tietokantaan.
...
//   korjtarfl = FALSE;
   ...
   // struktuuri erot kertoo l�ydetyt poikkeamat
				...
				if (emitfl &&
					...
					if ((ebadge[ibdg] = kilp.ostiet[os].badge[ibdg]) != 0) {
						...
						if (os1 == Sarjat[ekilp->sarja].osuusluku) {
							...
							if (ekilp->ostiet[os].badge[ibdg] == 0 && ep >= 0) {
								// Tee muutos my�s emittietueeseen, jos on l�ydetty
								em.badge = 0;
					...
					if (ekilp->ostiet[os].badge[ibdg]) {
						...
						if (addbadge(ekilp->ostiet[os].badge[ibdg], ekilp->kilpno, d, ibdg, kysyfl))
							...
							{
							// Taulukon p�ivitys onnistui. Tee muutos my�s emittietueeseen, jos on l�ydetty
							em.badge = ekilp->ostiet[os].badge[0];
							if (kilp.kilpno != ekilp->kilpno) {
								// P�ivit� my�s linkki kilpno -> tietue, jos kilpailijan numero vaihtuu
								setEmitJarr(kilp.kilpno, os, -1);
		  ...
		  for (os = 0; os < Sarjat[kilp.sarja].osuusluku; os++) {
			...
			if (comfl)
				...
				for (os = 0; os < Sarjat[kilp.sarja].osuusluku; os++) {
					...
					// luokka & 4,     l�het��n muut osuustiedot, koodit sis�lt�� sis�lt�maskin
		 ...
		 for (int ibdg = 0; ibdg < (kilpparam.kaksibadge ? 2 : 1); ibdg++) {
			 if (kilp->ostiet[os].badge[ibdg]) {
				if (addbadge(kilp->ostiet[os].badge[ibdg],
				   ...
				   sprintf(msg,"Emit-koodi %ld jo k�yt�ss�. Koodia ei tallennettu",
