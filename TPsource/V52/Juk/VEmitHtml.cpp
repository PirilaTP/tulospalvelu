...
// EMIT-leimatietojen HTML-tulostus viestikilpailussa sisältäen sarjan otsikkorivit.
   ...
   INT32 tl, tl0 = TMAALI0;
	...
	if (kilp.Maali(0, 0) != TMAALI0 ||
		...
		if (tulprm->language == 0) {
			...
			if (tulprm->rastivali)
				tulprm->writehtml(L"<em>Rastikoodi:</em> Rastiv�lin aika</th>");
			else
...
//				wcscat(line, L"<th>Maali</th></tr>\n");
				tulprm->writehtml(line);
				if (tulprm->rastivali) {
					tulprm->writehtml(L"<tr><th></th><th></th><th>Rastiv�livertailu</th><th><em>Lkm</em> ja paras aika</th><th></th>");
					line[0] = 0;
