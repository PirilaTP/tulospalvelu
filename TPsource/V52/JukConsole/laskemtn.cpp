...
// Viestikilpailun pisteiden laskenta ja suorituskaavat erillisessä konsoliversiossa.
...
INT haebdg(INT32 badge, int *toinen)
   {
	...
	// bsrchint palauttaa sijainnin kertovan negatiivisen arvon, ellei tarkkaa osumaa l�ydy
...
INT leimat(void)
   ...
   if (luentatesti) {
      ...
      if (luentatesti == 2) {
         ...
         if (alkut) {
            ch = ' ';
            selectopt("J)atka testi�, A)loita testaus alusta", "JA", &ch);
            }
         if (ch == 'A') {
            ch = ' ';
            selectopt("A)loita heti ensimm�isest� kilpailijasta, V)alitse aloitushetki", "AV", &ch);
            if (ch == 'A') {
               ch = ' ';
               selectopt("T)iedoston EMITREK.LST mukaiset ajat, V)akiov�liset ajat", "TV", &ch);
               if (ch == 'V') {
                  clrln(ySize-3);
               ...
               if (ch == 'M') {
                  clrln(ySize-3);
                  vidspmsg(ySize-3, 0, 7, 0, "Anna ensimm�isen kilpailijan lukemisaika");
                  alkut = T_TIME(biostime(0,0));
      ...
      for (r = 0;; r++) {
         ...
         clrln(ySize-3);
         if (luentatesti != 2) {
            vidspmsg(ySize-3,0,7,0, "Odottaa Emit-koodia - Esc: keskeyt� odotus");
            for (;;) {
...
//			os = tulkOsuuskoodi(sarjaKno(kno), wst);
         ...
         if (luentatesti == 2) {
            badge = bdg;
            vidspmsg(ySize-3,0,7,0, "Esc : Keskeyt� testi");
            for (;;) {
               ch = readkbd(&ch, 0, spoolfl);
               if (ch == ESC) {
                  return(0);
      ...
      for (i = 0; i < 2;) {
         ...
         switch (i) {
            case 0 :
			   ...
			   if (kilpparam.maxnosuus == 1 && kilpparam.ntosuus < 10) {
				   ...
				   if (strlen(line) == 6 && line[0] == '%') {
	                  ...
	                  kno = atol(line+1);
						...
						naytatulos(0);
						sprintf(msg, "Virheellinen merkkijono viivakoodista tai n�pp�imilt�: %s", line);
						writeerror(msg, 0);
						}
					}
				else {
               ...
               if (os < 1 || os > kilpparam.osuusluku || (d = getpos(kno)) <= 0) {
						asetavalo(2);
                  vidspmsg(r2, 27, 0, 7, "Joukkue tai osuus v��r� -lue uudelleen");
                  continue;
                  }
               clrtxt(r2, 23, 65);
               vidint(r3, 12, 4, kno);
               ...
               if (os >= Sarjat[kilp.sarja].osuusluku) {
                  vidspmsg(r2, 27, 0, 7, "Osuus v��r� -lue uudelleen");
				  asetavalo(2);
                  continue;
                  }
               entkilp = kilp;
               ...
               for (os1 = 0; os1 < Sarjat[kilp.sarja].osuusluku; os1++) {
				...
				if (toupper(kilp.ostiet[os].seuranta) == 'G') {
					viwrrect(r5+kilpparam.osuusluku,0,r5+kilpparam.osuusluku+2,64,
						"                                                                 "
						"      KILPAILIJA OSALLISENA GPS-SEURANTAAN - HAE L�HETIN         "
						"                                                                 ", 0, 3, 0);
					}
/*
 ...
 */
               if (kilp.ostiet[os-1].badge == 0) {
				  ...
				  if (aukkovaroitus) {
	                  erbeep();
		              selectopt("Edellisell� osuudella ei korttia. K)irjaa, E)i kirjata, L)opeta varoitus",
			             "KEL", &ch);
					  if (ch == 'L') {
						 aukkovaroitus = 0;
						 ch = 'K';
               ...
               if (kilp.Maali(os, 0) != TMAALI0 || kilp.Tark(os, true) != '-') {
				  ...
				  asetavalo(2);
                  erbeep();
                  selectopt("Osuudella jo tulos tai muu suoritusmerkint�. Kirjataanko silti (K/E)",
                     "KE", &ch);
                  if (ch == 'E')
                     continue;
                  }
            ...
            case 1:
               ...
               if (uusi_emit) {
                  ...
                  if (!salli_emit_ensin) {
                     ...
                     sprintf(line, "Lue uusi kortti tai P)oimi edellinen luettu:"
                        " %ld, Esc: keskeyt�", ed_emit);
                     vidspmsg(r1,0,0,7,line);
                     }
                  }
               else {
                  ed_emit = 0;
                  vidspmsg(r1,0,0,7,"Lue Emit-kortti, K)orjaa tieto n�pp�imist�lt�, Esc: keskeyt�");
                  }
               clrln(ySize-3);
			   ...
			   if (badge == 200) {
				...
				if (esta_uusikaytto && kno1 > 0) {
					...
					if (vd > 0) {
						...
						if (kno1 == kno && vkilp.ostiet[os].badge[0] == badge) {
							viwrrect(r5+kilpparam.osuusluku,0,r5+kilpparam.osuusluku+4,64,
								...
								"       PAINA ESC JA SIIRRY SEURAAVAN KILPAILIJAN K�SITTELYYN     "
								"                                                                 ", 0, 3, 0);
							i = 0;
					...
					if (vd > 0) {
						sprintf(line, "          Kortti k�yt�ss� joukkueen %d osuudella %s                ", kno1, osuuskoodi(vkilp.sarja, vos, 0, 0));
						viwrrect(r5+kilpparam.osuusluku,0,r5+kilpparam.osuusluku+4,64,
							"                                                                 "
							"                                                                 "
							"                                                                 "
               ...
               if (!esta_uusikaytto && kno1 && kno1 != kno) {
                  sprintf(line, "Emit-kortti jo k�yt�ss� joukkueella %d.", kno1);
                  kbflush();
                  writeerror(line, 0);
                  ch = 'E';
                  kbflush();
...
//                  rembadge(kno, os, d);
               if (ch != 'E')
                  kilp.ostiet[os].badge[0] = badge;
               if (ch == 'H')
                  selectopt("H)yv�ksy tiedot,  Esc: peruuta ja lue seuraava viivakoodi", "H\x1b", &ch);
               if (ch == 'H' || ch == 'K') {
