...
FormItem  FormList[] = {
	...
	{(TForm **)&FormLahestyjat[0], L"L‰hestyj‰t 1"},
	{(TForm **)&FormLahestyjat[1], L"L‰hestyj‰t 2"},
	{(TForm **)&FormLahestyjat[2], L"L‰hestyj‰t 3"},
	{(TForm **)&FormLahestyjat[3], L"L‰hestyj‰t 4"},
...
void __fastcall TFormMain::Initialisoi(wchar_t *kilphak, wchar_t *cfgflnm)
{
	...
	if (aloitus(0, 0, cfgflnm)) {
		writeerror_w(L"Siirtyminen pyydettyyn toimintatilaan ei onnistunut. Vain k‰ynnistysasetusten muuttaminen sallittua", 0);
		return;
	}
	if (ikkunaots[0])
	...
	if (ToimintaTila == 1) {
		...
		LbVaihe->Caption = sisaanluenta > 1 ? L"Vaihe: Sis‰‰nluenta" : L"Tulospalvelutila";
...
void __fastcall TFormMain::TallennaIkkunatV(UnicodeString FlNm)
{
	...
	if (tallfl->IsOpen()) {
		...
		tallfl->put_wxml_s(L"laji", L"p‰‰kaavake", 0);
