...
yhteyksissä käytetään yleensä UDP-protokollaa. TCP-protokolla
on käytettävissä, kun UDP
ei toimi esimerkiksi kännykkäyhteyden kautta. Myös sarjaporttien
ja nollamodeemikaapeleiden kautta tapahtuva tiedonsiirto toimii ohjelmissa edelleen. TCP-protokollaa voidaan käyttää
myös tietojen lähettämiseen XML-tietueina ulkoiselle palvelimelle.
...
Parametrina vastaip voidaan yhteyden toisessa päässä antaa myös sana AUTO.
Kun näin menetellään odottaa kyseinen kone UDP-sanomaa porttiin omaportti ja
käyttää jatkossa vastaanotetun sanoman lähettäjän ip-numeroa. Tämä ip-numero
voi lisäksi muuttua ohjelman toimiessa, sillä ohjelma käyttää aina viimeisen
porttiin omaportti vastaanottamansa UDP-sanoman lähettäjän ip-numeroa. Tämä
menettely on hyödyllinen sekä parametrien yksinkertaistamiseksi että tilanteessa,
jossa yhteyden toisen osapuolen ip-numeroa ei tiedetä ennalta tai se voi jopa vaihdella.
Tällaiseen tilanteeseen päädytään lähinnä silloin, kun yhteys otetaan internetin
kautta käyttäen julkista palveluntarjoajaa. Toisessa päässä on vastapuolen
ip-numero kuitenkin aina ilmoitettava, jotta yhteys voisi muodostua.
...
- Rakenna fyysinen verkko, joka koostuu yhdestä tai
  ...
  Mb/s ethernet-verkko lienee riittävä suurimpiinkin tulospalvelujärjestelmiin.- Kaikki konfigurointimuutokset tehdään verkkokorttiin liittyviin
    tcp/ip-asetuksiin (lukuun ottamatta niitä tapauksia, joissa yhteys
...
- **keskeytä ja uudelleen käynnistä
  tiedonsiirrto**
  joko kokonaan tai joko lähtevien tai saapuvien sanomien osalta
  ja joko kaikkien yhteyksien tai yksittäin valittavan yhteyden
  osalta.

  - **pyydä aiemmin lähetettyjen sanomien
    uudelleenlähetys**
    valiten minuutteina jakson, jota pyyntö koskee

    - **tarkastele tiedonsiirron
      varmistustiedoston COMFILE.DAT sisältöä**,
      josta selviää, millaisia sanomia kyseessä oleva
      tietokone on eri yhteyksiin lähettänyt.

      - **lähetä kilpailijatietoja XML-tietueina TCP-protokollaa käyttäen** ulkoiseen
        palveluun, joka käyttää tällaisia tietoja. Tämä tapahtuu yhteyteen, jonka
        tyypiksi on määritelty TCPX. Ohjelma lähettää tällaiseen yhteyteen, myös
        jokaisen uuden tulostiedon omana sanomanaan.- **lähetä tiedostoja** tulospalveluverkon UDP-yhteyksien kautta

Tiedostojen siirto on käytettävissä vain,
kun ohjelmalle on annettu parametri
...
missä x on yhden yhteyden numero. Edellinen muoto ottaa tiedostojen siirron käyttöön kaikissa
UDP-yhteyksissä, jälkimmäinen vain yhdessä yhteydessä.

Parametri

LÄHTIEDOSTOTx=0

voidaan antaa kaikkia yhteyksiä koskevan parametrin
jälkeen poistamaan tiedostosiirron yhteyteen x.

Parametrin loppuun voidaan lisätä täsmennys /K
.

Ellei ole annettu lisätäsmennystä /K
, siirretään tiedosto aina joko ohjelman
kilpailutietokansiosta tai sen
alikansiosta vastaanottavan koneen
vastaavaan kansioon. Vastaanottavan alikansion on oltava ennalta olemassa,
ohjelma ei luo uutta kansiota.

Kun lisätäsmennys /K
 on annettu,
käyttää ohjelma tiedoston
täyttä hakemistopolkua sellaisenaan.
Tällöin siirto edellyttää, että vastaanottavalla koneella on samoin
nimetty hakemisto saman levyasematunnuksen alla.
