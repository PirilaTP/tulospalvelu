...
// Viestikilpailun konsoliohjaimen tulostusfunktiot: näyttö, listaukset ja tiedostotulostus.
...
int luemuotoilu(int kysy)
   ...
   if (kysy) {
	  ...
	  if ((luetfmtf = _wfopen(luetfnm,L"rt")) == NULL) {
		 erbeep();
		 writeerror("Tulostusmuotoilutiedostoa ei l�ydy!",0);
		 return(0);
		 }
	  }
   clrln(ySize-1);
...
//   char av;
   ...
   if (luemuotoilu(1)) return;
	...
	if (lsttulparam.printer == GDIPRINTER) {
		vidspmsg(12,0,7,0,"K�yt�ss� Windowsin kirjoitinajurit. Kohdistus rivin tuhannesosina");
		}
	else
#endif
		{
		...
		viwrrect(13,4,17,51,
			...
			"3 : HP Laserjet kohdistus yksik�iss� 1/72 tuumaa"
			"4 : Postscript kohdistus yksik�iss� 1/72 tuumaa "
			"5 : Proprinteremulointi, yksik�iss� 1 merkki    "
			,7,0,0);
	//      "6 : Canon laser kohdistus yksik�iss� 1/72 tuumaa"
   ...
   vidspmsg(2,0,7,0,"Anna osuuksittaisen paperitulostuksen kenttien "
	...
	if (lsttulparam.printer == GDIPRINTER)
	   ...
	   sprintf(sline,"Sijainti %s, leveys merkkein�",
		   (lsttulparam.printer == LJETII || lsttulparam.printer == POSTSCRIPT) ? "yksik�iss� 1/72 in" :
			"merkkein�");
   ...
   for (i = 0; i < 3; i++) {
      ...
      if (ch == ESC) break;
      if (i == 0) {
         vidspmsg(2,0,7,0,"Anna osuuksittaisen n�ytt�tulostuksen"
            " kenttien sijainnit");
         fld = dspflds;
         clrln(ySize-5);
			}
...
static void tiedostovalinnat(tulostusparamtp *tulostusparam)
	...
	if (emitfl) {
		wcscpy(msg, L"Tulokset T)eksti, C)SV, H)TML, X)ML, E)mit-v�liajat (my�s XML)");
		wcscpy(opts, L"TCHXE");
		}
	else {
